...
public class ImageInfoDao {
	...
	public ImageInformation getById(String rowId) {
		return template.get(ImageSearchJob.imageInfoTable, rowId, new RowMapper<ImageInformation>() {

			@Override
			public ImageInformation mapRow(Result result, int rowNum)
					throws Exception {
				
				ImageInformation image = rowToImage(result);
	...
	public List<ImageInformation> getAll() {
		return template.find(ImageSearchJob.imageInfoTable, ImageSearchJob.COLUMN_FAMILY, new RowMapper<ImageInformation>() {
